...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${apiBaseUrl}/api/v1/query/baseline`, {
      method: "POST",
    ...
    if (upstream.status === 404) {
      return NextResponse.json(
        {
          detail: "Analytics API đang chạy nhưng thiếu POST /api/v1/query/baseline. Hãy dừng backend cũ và khởi động apps.api.main:app từ repository root.",
        },
        { status: 502 },
      );
    }
    ...
    return NextResponse.json(
      {
        detail: "Không kết nối được Analytics API. Hãy chạy backend tại localhost:8000.",
      },
      { status: 502 },
    );
  }
